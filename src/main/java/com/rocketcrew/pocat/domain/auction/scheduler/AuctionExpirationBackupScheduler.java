...
import com.rocketcrew.pocat.domain.auction.service.AuctionExpirationRedisService;
...
public class AuctionExpirationBackupScheduler {
    ...
    // 매일 오후 8시에 Redis 만료 이벤트로 처리되지 않은 만료 경매를 DB 기준으로 보정 종료한다.
    ...
    public void closeExpiredAuctions() {
        ...
        for (Auction auction : expiredAuctions) {
            try {
                boolean closed = auctionLifecycleService.closeExpiredAuction(auction.getId());
                if (closed) {
                    auctionExpirationRedisService.deleteShadowKey(auction.getId());
                }
            } catch (Exception e) {
