...
`status`	VARCHAR(30)	NOT NULL	COMMENT 'PENDING / COMPLETED / REFUNDED',
`settled_at`	TIMESTAMP	NULL	COMMENT '정산 완료 시각',
...
`id`	BIGINT	NULL,
`user_id`	BIGINT	NOT NULL,
`type`	VARCHAR(50)	NOT NULL	COMMENT 'BID_OUTBID/AUCTION_WON/AUCTION_LOST/INSPECTION_PASSED/INSPECTION_FAILED/SHIPPING/SHIPPING_COMPLETED/ORDER_CANCELLED/SETTLEMENT_CREATED/SETTLEMENT_COMPLETED/PAYMENT_COMPLETED/AUTO_PAYMENT_FAILED/DIRECT_PAYMENT_FAILED/PAYMENT_FINAL_FAILED/ESCALATED_PAYMENT_OPPORTUNITY/REFUND_REQUESTED/REFUND_APPROVED/REFUND_REJECTED/AUCTION_ACTIVATED/AUCTION_CANCELLED/AUCTION_SOLD/BID_CREATED',
...
`user_role`	VARCHAR(20)	NOT NULL	COMMENT 'USER/ADMIN',
...
`status`	VARCHAR(30)	NOT NULL	COMMENT 'PAYMENT_PENDING/DIRECT_PAYMENT_FAILED/AUTO_PAYMENT_FAILED/CANCELLED/PAYMENT_COMPLETED/ORDER_COMPLETED/REFUNDED',
`cancel_reason`	VARCHAR(255)	NULL,
`payment_deadline`	TIMESTAMP	NULL,
`bidder_rank`	INT	NULL,
`order_type`	VARCHAR(20)	NOT NULL	COMMENT 'AUCTION/BUYOUT',
`created_at`	TIMESTAMP	NOT NULL,
`updated_at`	TIMESTAMP	NOT NULL
);
...
`series_id`	BIGINT	NULL	COMMENT 'series FK',
`pokemon_set_id`	BIGINT	NULL	COMMENT 'pokemon_sets FK',
`pokemon_id`	BIGINT	NULL	COMMENT 'pokemon FK',
`card_number`	VARCHAR(20)	NOT NULL	COMMENT '001 등 카드번호',
`rarity`	VARCHAR(50)	NOT NULL,
`category`	VARCHAR(20)	NOT NULL	COMMENT 'POKEMON/TRAINERS/ENERGY/UNKNOWN',
`grade`	VARCHAR(20)	NOT NULL	COMMENT 'PSA_10/PSA_9/BGS_10',
`image_url`	VARCHAR(500)	NULL,
`source`	VARCHAR(20)	NOT NULL	COMMENT 'TCGDEX/MANUAL',
`status`	VARCHAR(20)	NOT NULL	COMMENT 'ACTIVE/PENDING/REJECTED',
...
`payment_method`	VARCHAR(50)	NULL	COMMENT 'CARD/KAKAO_PAY/TOSS etc',
`status`	VARCHAR(20)	NOT NULL	COMMENT 'PENDING/COMPLETED/FAILED/REFUNDED/CANCELLED/CANCEL_HTTP_ERROR',
`paid_at`	TIMESTAMP	NULL,
`billing_key_requested_at`	TIMESTAMP	NULL,
`created_at`	TIMESTAMP	NOT NULL
);
...
`status`	VARCHAR(20)	NOT NULL	COMMENT 'REQUESTED/PROCESSING/COMPLETED/REJECTED/FAILED_RETRYABLE/FAILED_FINAL',
...
`status`	VARCHAR(20)	NOT NULL	COMMENT 'LEADING/OUTBID/WON/LOST/CANCELLED',
`created_at`	TIMESTAMP	NOT NULL
);
...
`owner_left`	BOOLEAN	NOT NULL,
`guest_left`	BOOLEAN	NOT NULL,
`created_at`	TIMESTAMP	NOT NULL,
`updated_at`	TIMESTAMP	NOT NULL
);
