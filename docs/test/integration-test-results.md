...
| **Date** | 2026-06-02 |
| **Branch** | `dev` |
| **대상** | 결제 · 자동결제 · 입찰 · 정산 · 환불 동시성 통합 테스트 |
...
5개 테스트 클래스 모두 동일한 `@MockBean` 세트를 사용하므로 Spring이 컨텍스트를 재사용한다.
전체 실행 시간 단축에 기여한다.
...
| 영역 | 이유 | 제안 방향 |
|------|------|-----------|
| Kafka 이벤트 멱등성 | 실제 Kafka 브로커 필요 | Testcontainers Kafka 도입 |
| 즉시구매 동시성 (`buyout()`) | Redisson + REQUIRES_NEW 트랜잭션 조합 복잡도 | AuctionBuyoutService 전용 테스트 |
| confirmPayment 중복 완료 차단 | PortOne Mock 응답 설정 필요 | PortOneClientService stubbing 확장 |
| 환불 재시도 (`retryRefund()`) | 스케줄러 + 비관적 락 조합 | ScheduledExecutorService 시뮬레이션 |
