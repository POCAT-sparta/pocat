...
1. **HikariCP 커넥션 풀 (기본값 10)** — 동시 DB 요청 10개 초과 시 커넥션 대기 → 타임아웃 → BP
...
| **로컬 환경 Breaking Point** | ✅ **readers ~300 VU** | `p(95) = 593ms`, BP 신호 1건 |
...
| 서비스가 수용 가능한 최대 동접 수 | 로컬 BP 탐색 수행 (300 VU 한계 확인). 단, 프로덕션 용량 계획은 클라우드 환경 재측정 필요 |
