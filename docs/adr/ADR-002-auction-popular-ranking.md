...
### 3. popularityScore 응답 비노출

**결정**: `popularityScore`는 API 응답에 포함하지 않고, 서버 내부 정렬 기준으로만 사용한다.

- 사용자용 경매 목록과 인기 경매 목록 응답 필드를 동일하게 유지해 클라이언트의 목록 UI 재사용성을 높인다.
- 점수 산식과 가중치는 랭킹 정책이므로 외부 API 계약으로 고정하지 않는다.
- 사용자가 화면에서 확인해야 하는 공개 지표는 `likeCount`이며, 입찰 수와 내부 점수는 현재 응답 범위에서 제외한다.
...
### 5. 스케줄러 조회 범위: ACTIVE 경매 전체, ORDER BY started_at DESC
