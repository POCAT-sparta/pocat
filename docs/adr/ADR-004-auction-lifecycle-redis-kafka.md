...
### 1. 경매 활성화는 매일 오후 7시 스케줄러로 처리한다.

**결정**: 매일 19:00 KST에 `APPROVED` 상태 경매를 조회하여 `ACTIVE`로 전환한다.

- 스케줄러: `AuctionActivationScheduler`
- 실행 시각: `0 0 19 * * *`, `Asia/Seoul`
- 상태 전이: `APPROVED -> ACTIVE`
- 시작 시각: `startedAt = now`
- 종료 시각: `endedAt = now + 3일`
- 활성화 성공 시 `AuctionActivatedEvent` 발행

경매 생성 또는 검수 통과 시점에 즉시 시작하지 않고, 운영 정책상 매일 오후 7시에 일괄 시작한다.
...
shadow key는 TTL을 주지 않는다. 종료 처리가 성공한 경우에만 애플리케이션이 직접 삭제한다. 따라서 Redis 만료 이벤트를 받지 못하거나 종료 처리 중 오류가 발생하면 shadow key가 남아 미처리 경매 추적에 사용할 수 있다.
...
**결정**: 매일 20:00 KST에 DB 기준으로 만료된 `ACTIVE` 경매를 조회해 보정 종료한다.

- 스케줄러: `AuctionExpirationBackupScheduler`
- 실행 시각: `0 0 20 * * *`, `Asia/Seoul`
...
- **장애 보정**: Redis 이벤트가 유실되어도 오후 8시 백업 스케줄러가 DB 기준으로 보정한다.
...
- **백업 스케줄러 지연**: Redis 이벤트가 동작하지 않으면 종료 처리는 오후 8시 스케줄러까지 지연될 수 있다.
...
- `AuctionExpirationBackupScheduler.java` - 매일 오후 8시 만료 경매 보정 종료
